...
import { fetchAllTreatmentTypes, deleteTreatmentType } from "@/services/medicals/TreatmentTypeService";
import { useLoaderTablePagination } from "@/helper/loaderTablePagination";
import ShowTreatmentTypeModal from "./ShowTreatmentTypeModal";
        ...
        <div className="col-lg-12">
            <div className="card">
                <div className="card-header border-bottom">
                    <h4 className="card-title mb-3">Daftar Jenis Perawatan</h4>
                    <div className="d-flex align-items-center">
                        ...
                        <Link
                            href="/page/clinic/treatments/create"
                            ...
                            <table className="table striped-table mb-0">
                                ...
                                <tbody>
                                    {data?.content?.length > 0 ? (
                                        data.content.map((treatment, idx) => (
                                            <tr key={treatment.id}>
                                                <td>{idx + 1}</td>
                                                ...
                                                <td className="text-center">
                                                    <Link
                                                        ...
                                                        title="Lihat"
                                                        onClick={() => setShowModal(treatment.id)}
                                                    >
                                                        <Icon icon="iconamoon:eye-light" />
                                                    </Link>
                                                    <Link
                                                        className="w-32-px h-32-px me-8 bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center"
                                                        href={`/page/clinic/treatments/${treatment.id}`}
                                                        title="Edit"
                                                    >
                ...
                {showModal && (
                    <ShowTreatmentTypeModal id={showModal} onClose={() => setShowModal(null)} />
                )}
            </div>
        </div>
    );
