...
import { fetchAllPatients, deletePatient } from "@/services/medicals/PatientService";
        ...
        <div className="col-lg-12">
            <div className="card">
                <div className="card-header border-bottom">
                    <h4 className="card-title mb-3">Daftar Pasien</h4>
                    <div className="d-flex align-items-center">
                        ...
                        <Link
                            href="/page/clinic/patients/create"
                            ...
                            <table className="table striped-table mb-0">
                                ...
                                <tbody>
                                    {data?.content?.length > 0 ? (
                                        data.content.map((patient, idx) => (
                                            <tr key={patient.id}>
                                                <td>{idx + 1}</td>
                                                <td>{patient.identity.full_name}</td>
                                                <td>{patient.identity.gender || '-'}</td>
                                                <td className="text-center">
                                                    <span className="bg-success-focus text-success-main px-32 py-4 rounded-pill fw-medium text-sm">
                                                        {patient.is_active ? "Tidak Aktif" : "Aktif"}
                                                    </span>
                                                </td>
                                                <td className="text-center">
                                                    <Link
                                                        ...
                                                        title="Lihat"
                                                        onClick={() => setShowModal(patient.id)}
                                                    >
                                                        <Icon icon="iconamoon:eye-light" />
                                                    </Link>
                                                    <Link
                                                        className="w-32-px h-32-px me-8 bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center"
                                                        href={`/page/clinic/patients/${patient.id}`}
                                                        title="Edit"
                                                    >
                ...
                {showModal && (
                    <ShowPatientModal id={showModal} onClose={() => setShowModal(null)} />
                )}
            </div>
        </div>
    );
