...
import { fetchAllOrganizations } from "@/services/cores/OrganizationService"; 
import { useLoaderTablePagination } from "@/helper/loaderTablePagination"; 

const ListOrganization = () => {
    // Gunakan hook loaderTablePagination
    const {
        data, // Data tabel
        loading, // Status loading
        pagination, // Info tentang pagination
        fetchData, // Fungsi untuk mem-fetch ulang data
        applyFilters,
        goToPage, // Fungsi navigasi halaman
        handleSearch, // Fungsi untuk pencarian
    } = useLoaderTablePagination({
        fetchFunction: async ({ page, perPage, search }) =>
            await fetchAllOrganizations({
                page: page - 1, // Page API 0-based, tapi app ini 1-based
                size: perPage,
                name: search || undefined, // Kirimkan search (bila ada)
            }),
        initialPage: 1,
        initialPerPage: 10, // Jumlah data per halaman
    });
    ...
    const onSearchSubmit = (e) => {
        e.preventDefault();
        handleSearch(searchQuery); // Memanggil fungsi handleSearch dari hook
    };
        ...
        <div className="col-lg-12">
            <div className="card">
                <div className="card-header border-bottom">
                    {/* Card Title */}
                    <h4 className="card-title mb-3">List Organization</h4>
